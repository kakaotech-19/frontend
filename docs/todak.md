## UI 가이드

### flowbite react사용

- modal, carousel, accordion 같은 효과 이미 구현되어있는 라이브러리임
- UI 직접 구현한거 하나도없고 전부 flowbite에서 가져와서 스타일만 만짐
- 뭔가 효과넣고 싶은데, 못찾겠으면 leo에게 문의, 구현되어 있을 수 있음
- headless ui, mui등등 익숙한 라이브러리가 존재하면 마구 추가해서 써도 전혀 문제 없음

## Next JS 가이드

리소스이 표현법 : /[유저]/[날짜]/write/ nested
wite? user="" & date="" shallow
CSR 빌드를 먼저한다음에 유저브라우저에 코드를 제공 []

shallow routing: 페이지 이동시 전체 페이지를 다시 렌더링하지 않고, 변경된 부분만 렌더링하는 방식

### 기본 규칙 (중요 ⭐️)

- CSR을 위해서 화면 상단에 'use client' 지시어 사용
- layout.tsx 하위 디렉토리 모두에서 공통으로 화면에 그려짐
- () 사용하면 경로에 포함되지않은 디렉토리 그룹
- APP 디렉토리 안의 파일이 서비스의 경로가 됨
- page.tsx는 디렉토리마다 반드시 1개씩 존재해야함
- next.config.mjs에서 활성 포트, 환경변수, 빌드 설정 등을 설정할 수 있음
- 환경변수 NEXT_PUBLIC 접두사로 시작안하면 못알아먹음
- image 나 audio src 불러올땐 public을 루트 경로로 인식함

### 라우팅

- loading, error 페이지 커스텀 가능하나 굳이 하지 않음
- 동적 URL 필요할경우 []로 사용하면 가능하지만, shallow routing방식을 사용하고 있기 때문에 굳이 활용하지 않음
- router를 사용할때 next/router는 서버의 라우터임. 반드시 next/navigation을 사용할 것
- router를 사용할때 경로를 하드코딩하지 말고 shared/routes에 명시한 뒤 변수로 사용할 것
- 쿼리스트링을 페이지에서 받아올 때 useSerchParams()를 사용함

## Redux Chunk 가이드

- 써도되고 안써도됨. zustand쓰고 싶다면, 그대로 두고 새로 추가하는 식으로 사용 가능
- dispatch()안에 함수를 넣어야 동작함
- 스토어에서 변수를 불러올땐 selector()를 사용함
- slice는 도메인을 분류함
- extraReducers 파일에서는 비동기 함수의 요청, 응답을 관리함

## 토닥 디렉토리 구조

- Domain: 도메인을 분류하는 최상위 기준

  - components: 도메인의 컴포넌트를 관리하는 파일
  - slices: 전역상태관리에서 사용하는 상태를 관리하는 파일
  - dto: dto 써도 되고 굳이 안써도 됨
  - hooks: 커스텀 훅
  - functions: 보통 예외처리나 유틸함수가 들어있음
  - types: 타입스크립트 타입, 인터페이스
  - constants: 상수값, enum등

- shared: 여러 도메인에서 공통으로 사용하는 파일

  - axios: http 요청 라이브러리

- mocks: 테스트용 목업 서버 데이터

## 명령어

- `pnpm add --save-dev 패키지`: 개발 의존성 추가
- `pnpm add --save 패키지`: 의존성 추가

### 자주쓰는 명령어

- `pnpm install`: 노드 모듈 설치
- `pnpm dev`: 개발 서버 실행
- `pnpm build`: 프로덕션 빌드

### 실습 내용

- api문서를 보고 redux Chunk
- Modal 추가하고 열기
- 데이터 불러와서 렌더링하기

### 실습 시나리오

- 새로운 Follow 방 기능을 만들어야합니다. API 문서의 요청을 보고, redux chunk를 사용하여 구현해주세요.
- API는
